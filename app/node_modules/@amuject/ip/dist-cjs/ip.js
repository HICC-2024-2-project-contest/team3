...
const ___dirname = _path.default.dirname((0, _url.fileURLToPath)(require('url').pathToFileURL(__filename).toString()));
const bogonIPs = JSON.parse(_fs.default.readFileSync(_path.default.resolve(___dirname, '../data/bogon-ips.json')).toString());
const regexp = {
  subnet: new RegExp(/\/([0-9]+)/),
  v4String: new RegExp(/^((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])/),
...
var Type;
(function (Type) {
  Type[Type["IPv4"] = 0] = "IPv4";
  Type[Type["IPv6"] = 1] = "IPv6";
})(Type || (Type = {}));
class IP {
  #ip = 0n;
  #subnet = 0n;
  ...
  toString() {
    return this.is4() ? this.#IPv4DemicalToString(this.#ip) : this.#IPv6DemicalToString(this.#ip);
  }
  toDemical() {
    return this.#ip;
  }
  ...
  to6() {
    if (this.is4()) {
      ...
      return new IP('0000:0000:0000:0000:0000:ffff:' + array[0].toString(16).padStart(2, '0') + array[1].toString(16).padStart(2, '0') + ':' + array[2].toString(16).padStart(2, '0') + array[3].toString(16).padStart(2, '0'));
    } else {
  ...
  getSubnetIPs() {
    ...
    for (let i = subnet.network.toDemical(); i <= subnet.broadcast.toDemical(); i++) {
      array.push(new IP(i));
    }
    return array;
  }
  in(i1, i2) {
    if (i1.hasSubnet()) {
      ...
      return subnet.range.start.toDemical() <= this.#ip && this.#ip < subnet.range.end.toDemical();
  ...
  #IPv6DemicalToString(ip) {
    const array = this.#DemicalToIPArray(ip);
    let string = '';
    for (let i = 0; i < array.length; i++) {
      string += array[i].toString(16).padStart(2, '0') + (i % 2 != 0 && i < array.length - 1 ? ':' : '');
    }
    return string;
  }
}
var _default = exports.default = IP;
